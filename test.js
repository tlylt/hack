...
export default {
		...
		setTimeout(() => {
			...
			hands.pointer.forEach((pointer, n) => {
				...
				if (hands.pinchState[n]?.[0] === "start") {
					...
					this.tweenScroll[n].x = this.origScrollLeft[
						n
					] = this.getTargetScrollLeft(this.$target[n]);
					this.tweenScroll[n].y = this.origScrollTop[
						n
					] = this.getTargetScrollTop(this.$target[n]);
	...
	getTarget($potTarget) {
		const styles =
			$potTarget && $potTarget.getBoundingClientRect
				? getComputedStyle($potTarget)
				: {};

		if (
			$potTarget &&
			$potTarget.scrollHeight > $potTarget.clientHeight &&
